...
<body>
  ...
  <script>
    ...
    function simplePick(item) {
      const level = item.recommendation.level;
      if (level === 'green') {
        return {
          tone: 'green',
          label: 'Most likely click this',
          title: 'Approve it if it looks normal.',
          body: 'Use this case for training. If anything looks wrong, use Needs Fix instead.',
          buttonClass: 'green',
          buttonText: 'Click this: Approve - use this',
          status: 'approved'
        };
      }
      return {
    ...
    function simplePickPanel(item) {
      const pick = simplePick(item);
      return \`
        <div class="simple-pick \${escape(pick.tone)}">
          <div>
            <span>\${escape(pick.label)}</span>
            <h3>\${escape(pick.title)}</h3>
            <p>\${escape(pick.body)}</p>
          </div>
          <button class="\${escape(pick.buttonClass)}" onclick="quickDecision('\${escape(pick.status)}')">\${escape(pick.buttonText)}</button>
        </div>
      \`;
    }

    function otherChoices(item) {
      const isRed = item.recommendation.level === 'red';
      const approveChoice = isRed
        ? ''
        : '<button id="approveBtn" class="green" onclick="quickDecision(\\'approved\\')">Approve - use this</button>';
      return \`
        <details class="other-choice-box">
    ...
    function quickDecision(status) {
      const item = currentCase();
      const notes = document.getElementById('notes');
      if (notes && !notes.value.trim()) {
        const defaultNote = defaultDecisionNote(item, status);
        if (defaultNote) notes.value = defaultNote;
      }
      saveDecision(status);
    }
    ...
    window.addEventListener('keydown', e => {
      ...
      if (activeView === 'review' && key === 'a') {
        const approveBtn = document.getElementById('approveBtn');
        if (approveBtn && !approveBtn.disabled) quickDecision('approved');
      }
      if (activeView === 'review' && key === 'f') quickDecision('needs_revision');
      if (activeView === 'review' && key === 'r') quickDecision('rejected');
      if (activeView === 'review' && key === 's') quickDecision('skipped');
