...
function comp(
  ...
  return {
    ...
    sales_price_per_gla: salePrice != null ? Math.round((salePrice / (1750 + compIndex * 80)) * 100) / 100 : null,
    net_adjustment: netAdjustment,
    net_adjustment_percent: salePrice && netAdjustment != null ? Math.round((netAdjustment / salePrice) * 1000) / 10 : null,
    gross_adjustment: netAdjustment == null ? null : Math.abs(netAdjustment) + 3500,
    gross_adjustment_percent: salePrice && netAdjustment != null ? Math.round(((Math.abs(netAdjustment) + 3500) / salePrice) * 1000) / 10 : null,
    ...
    condition,
    quality,
    gla_sqft: 1750 + compIndex * 80,
    total_rooms: 7 + compIndex,
    bedrooms: compIndex === 1 ? 3 : 4,
    bathrooms: compIndex === 3 ? 2.1 : 2,
    full_bathrooms: 2,
    half_bathrooms: compIndex === 3 ? 1 : null,
    site_size: compIndex === 1 ? "0.42 ac" : "0.55 ac",
    view: compIndex === 2 ? "N;Woods" : "N;Residential",
    location: "N;Suburban",
    property_rights: "Fee simple",
    sales_concessions: compIndex === 1 ? "None" : null,
    financing_concessions: compIndex === 2 ? "Conventional;0" : null,
    sale_date: saleDate,
    sale_date_raw: saleDate,
    contract_date: saleDate ? saleDate.slice(0, 7) : null,
    actual_age: 18 + compIndex * 6,
    year_built: 2006 - compIndex * 6,
    design_style: compIndex === 3 ? "DT1;Ranch" : "DT2;Traditional",
    basement_area_sqft: compIndex === 2 ? 900 : 0,
    basement_finished_sqft: compIndex === 2 ? 450 : null,
    basement_description: compIndex === 2 ? "900sf450sfwo" : "0sf",
    basement_finish: compIndex === 2 ? "450sf" : "0sf",
    functional_utility: "Average",
    heating_cooling: "FWA/CAC",
    energy_efficient: null,
    garage_carport: compIndex === 1 ? "2ga" : "2ga2dw",
    garage_spaces: 2,
    carport_spaces: null,
    porch_deck: compIndex === 3 ? "Patio" : "Porch",
    fireplaces: compIndex === 2 ? "1 Fireplace" : null,
    other_features: [],
